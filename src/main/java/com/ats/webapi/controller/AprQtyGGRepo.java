...
*/
    ...
    @Query(value = " SELECT t1.*,IFNULL(t2.all_apr_qty,0) as all_apr_qty " + 
    		...
    		"    LEFT JOIN (SELECT IFNULL(SUM(gg.apr_qty_acc),0) as all_apr_qty,gg.bill_detail_no FROM t_grn_gvn gg WHERE gg.grn_gvn_status=6 and gg.bill_detail_no In(12249) and gg.grn_gvn_id NOT IN (:grnGvnIdList) group by gg.bill_detail_no) t2 on t1.bill_detail_no=t2.bill_detail_no\n" + 
    		"", nativeQuery = true)
	List<AprQtyGG> getAprQtyGG(@Param("billDetailNoList") List<Integer> billDetailNoList,
			@Param("grnGvnIdList") List<Integer> grnGvnIdList);
