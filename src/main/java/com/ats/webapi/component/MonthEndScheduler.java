...
//*/2 * * * *
		...
		public void autoMonthEnd() {
			...
			catList.add(1);//catList.add(2);catList.add(3);catList.add(4);
			int frId=0;
			...
			for(int c=0;c<catList.size();c++) {
				...
				PostFrItemStockHeader stockHeader=postFrOpStockHeaderRepository.findByFrIdAndCatIdAndIsMonthClosed(frId,catList.get(c),0);
				if(stockHeader==null) {
	...
	System.err.println("str date " +strDate);
							...
							List<GetCurrentStockDetails> currentStockDetailList = stockDetailRepository.getMinOpeningStock2(stockHeader.getMonth(), Integer.parseInt(currentYear), frId, catList.get(c),
									strDate,getMonthFirstDate(), frList.get(f).getStockType(), getItemList(frId,catList.get(c)));
